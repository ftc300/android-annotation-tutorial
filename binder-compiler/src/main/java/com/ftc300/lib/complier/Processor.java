package com.ftc300.lib.complier;
            ...
            for (TypeElement typeElement : typeElements) {
                ...
                classBuilder.addMethod(MethodSpec.constructorBuilder()
                        ...
                        .addStatement("$N($N)",
                                ...
                                NameStore.Variable.ANDROID_ACTIVITY)
                        .addStatement("$N($N)","setFtcValue",NameStore.Variable.ANDROID_ACTIVITY)
                        .build());
                // add  my method that assign value
                MethodSpec.Builder assignValue = MethodSpec
                        .methodBuilder("setFtcValue")
                        .addModifiers(Modifier.PRIVATE)
                        .returns(void.class)
                        .addParameter(className, NameStore.Variable.ANDROID_ACTIVITY);
                for (VariableElement variableElement : ElementFilter.fieldsIn(typeElement.getEnclosedElements())) {
                    FTC300_CLASS bindView = variableElement.getAnnotation(FTC300_CLASS.class);
                    if (bindView != null) {
                               assignValue.addStatement("$N.$N =\"$L\"",
                                NameStore.Variable.ANDROID_ACTIVITY,
                                 variableElement.getSimpleName(),
                                bindView.value());
                    }
                }
                classBuilder.addMethod(assignValue.build());
